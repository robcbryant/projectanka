...
class FormProject(models.Model):
    ...
    modified_by = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='project_ref_to__user_modifier', blank = True, null=True,on_delete=models.SET_NULL)
...
class FormType(models.Model):
    ...
    def string_list_hierarchy(self):
        ...
        for aForm in self.form_set.filter(hierarchy_parent=None):
            print >> sys.stderr, aForm.get_hierarchy_label() + "<!-----"
    ...
    def get_thumbnail_type(self):
        if self.form_type.media_type == 3: #3D File
            ...
            if self.form_type.uri_prefix != None or self.form_type.uri_prefix != "":
                print >>sys.stderr, self.form_type.project.uri_thumbnail
                print >>sys.stderr, self.form_type.uri_prefix
                print >>sys.stderr, self.form_type.file_extension 
                print >>sys.stderr, self.form_name
...
class Permissions(models.Model):
    ...
    project = models.ForeignKey(FormProject, null=True, blank=True)
