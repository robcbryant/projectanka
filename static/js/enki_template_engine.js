        ...
        for (let i = 0; i < widgets.length; i++){
            ...
            switch (widget.type){
                ...
                case 'image-view':
                    ...
                    if(INITIALIZATION_CODE != 0)$newImageView.find('.search-box input')[0].addEventListener("input", function(){lookForReferenceForms(this, GV_projectPK, $(this).parent().parent().parent().attr('ref_pk'),CURRENT_FORMTYPE_PK)});
                ...
                case 'thumb-view':
                    ...
                    //Only add the listener if we aren't editing a template
                    if(INITIALIZATION_CODE != 0)$newThumbView.find('.search-box input')[0].addEventListener("input", function(){lookForReferenceForms(this, GV_projectPK, $(this).parent().parent().parent().attr('ref_pk'),CURRENT_FORMTYPE_PK);});
                ...
                for(var i = 0; i < RTYPE_LIST.length; i++){
                    ...
                    if (rtype.rtype == 'FRAT'){
                        ...
                        if(INITIALIZATION_CODE != 0){
                            ...
                            $newThumbView.find('.search-box input')[0].oninput = function(){  lookForReferenceForms(this, GV_projectPK, $(this).attr('ref_rtype'),$(this).attr('parent_rtype')) };
                        }
    ...
    function loadRVALSIntoWidgets(rtype_list){
        for (i = 0; i < rtype_list.length; i++){
           ...
           if ($currentWidget.hasClass('widget-frat')){
                ...
                //We use the loop here to iterate through the keys even though there is one, because there doesn't seem to be an efficient way
                //  --to just grab the first if any key in a JSON object key/val set
                //Set the value of the widget
                for (var key in currentRTYPE.rval){
                    $currentWidget.find('textarea').html(currentRTYPE.rval[key]);
                    $currentWidget.find('textarea').attr('name', 'frav__'+key);
                }
                ...
                $currentWidget.find('.search-box input')[0].oninput = function(){  lookForReferenceForms(this, GV_projectPK, $(this).attr('ref_rtype'),$(this).attr('parent_rtype')) };
